export default function StripePreview() {
  const stripes = [
    { label: "Blue",   color: "oklch(0.52 0.22 252)" },
    { label: "Purple", color: "oklch(0.52 0.25 285)" },
    { label: "Red",    color: "oklch(0.55 0.24 18)"  },
    { label: "Orange", color: "oklch(0.68 0.20 38)"  },
  ]
  ...
  return (
    <div>
      <div className="bg-background border-b border-border px-6 py-3 flex items-center gap-3 text-sm">
        ...
        <span className="font-medium">Stripe color mocks</span>
      </div>

      {stripes.map((stripe) => (
        <div key={stripe.label}>
          {/* Dark */}
          <Mock stripe={stripe} dark />
          {/* Light */}
          <Mock stripe={stripe} dark={false} />
        </div>
      ))}
    </div>
  )
...
function Mock({ stripe, dark }: { stripe: { label: string; color: string }; dark: boolean }) {
  ...
  return (
    <section style={{ backgroundColor: bg, color: fg, fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      <div className="px-6 py-1.5 text-xs font-semibold tracking-widest uppercase flex justify-between"
        style={{ backgroundColor: stripe.color, color: "oklch(0.97 0 0)" }}>
        <span>{stripe.label}</span>
        <span className="opacity-60">{dark ? "Dark" : "Light"}</span>
      </div>

      {/* Top stripe */}
      <div style={{ height: "3px", backgroundColor: stripe.color }} />
        ...
        <nav className="flex items-center justify-between py-5"
          ...
          <div className="flex gap-6 text-xs">
            {["Notes", "Experiments", "Projects", "About"].map((link, i) => (
              <span key={link} style={{
                color: i === 0 ? stripe.color : muted,
                borderBottom: i === 0 ? `1px solid ${stripe.color}` : "none",
